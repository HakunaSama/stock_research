...
export default function Watchlist() {
  const { selectedId, select } = useTerminal();
    ...
    <div className="flex h-full flex-col overflow-hidden rounded-lg border border-subtle bg-panel">
      <div className="flex items-center justify-between border-b border-subtle px-3 py-2">
        <div className="flex items-center gap-1.5">
          ...
          <span className="font-display text-xs font-600 text-ink">主板复选池</span>
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {watchlist.map((w, i) => {
          ...
          return (
            <motion.button
              key={w.code}
              onClick={() => select(w.code)}
              initial={{ opacity: 0, x: 12 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.35, delay: i * 0.03 }}
              className="flex w-full items-start gap-2 border-b border-subtle/60 px-3 py-2 text-left transition-colors hover:bg-elevated"
            ...
            >
              <div className="mt-0.5 shrink-0">
                {active ? (
                  ...
                  <Square size={13} className="text-ink-3" />
                )}
              </div>
